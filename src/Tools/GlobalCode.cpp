...
IDirect3DDevice9 * EnsureDev()
{
	if (g_Dev == NULL)
	{
		// Sposób inicjowania tego tutaj jest wziêty z DXSDK\Samples\C++\Direct3D\UVAtlas.

		IDirect3D9 *d3d = Direct3DCreate9(D3D_SDK_VERSION);
		if (d3d == NULL) throw Error("Nie mo¿na utworzyæ obiektu Direct3D.");
		...
		try
		{
			...
			HRESULT hr = d3d->CreateDevice(
				...
				&pp,
				&Dev);
			if (FAILED(hr)) throw DirectXError(hr, "Nie mo¿na utworzyæ urz¹dzenia Direct3D.");
			g_Dev.reset(Dev);
