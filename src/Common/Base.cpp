...
TimeMeasurer::TimeMeasurer()
{
		...
		timeval tv;
		gettimeofday(&tv, 0);
		m_Start = (int8)tv.tv_sec * 1000000 + (int8)tv.tv_usec;
	#endif
}
...
double TimeMeasurer::GetTimeD()
{
		...
		timeval tv;
		gettimeofday(&tv, 0);
		int8 DeltaTime = ((int8)tv.tv_sec * 1000000 + (int8)tv.tv_usec) - m_Start;
...
void RandomGenerator::RandData(void *OutData, size_t DataLength)
{
	...
	uint4 RemainingBytes = DataLength & 0x3;
...
UniqueGenerator::UniqueGenerator(uint4 First) :
	m_Next(First)
{
}

void UniqueGenerator::GetString(string *Out)
{
	UintToStr2(Out, m_Next, 8, 16);
	m_Next++;
}

void UniqueGenerator::GetString(string *Out, const string &Prefix)
{
	string s;
