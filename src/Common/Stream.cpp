...
// Jeœli to znak '=', zwraca 0xFE.
// Jeœli znak nieznany, zwraca 0xFF.
...
void CharWriter::WriteString(const string &s)
{
	...
	// Length bêdzie zmniejszany. Oznacza liczbê pozosta³ych do zapisania bajtów.
...
size_t VectorStream::Read(void *Data, size_t Size)
{
	if (m_Pos >= 0 && m_Pos <= (int)m_Size)
	{
		...
		throw Error(Format("Nie mo¿na odczytaæ # bajtów ze strumienia VectorStream - pozycja poza zakrsem (pozycja: #, rozmiar: #)") % Size % m_Pos % m_Size, __FILE__, __LINE__);
	return Size;
}
...
void VectorStream::MustRead(void *Data, size_t Size)
{
	if (m_Pos >= 0 && m_Pos + (int)Size <= (int)m_Size)
	{
		...
		throw Error(Format("Nie mo¿na odczytaæ (2) # bajtów ze strumienia VectorStream - pozycja poza zakrsem (pozycja: #, rozmiar: #)") % Size % m_Pos % m_Size, __FILE__, __LINE__);
}
...
void VectorStream::SetSize(size_t Size)
{
	...
	{
		m_Size = Size;
		// jeœli po³owa miejsca siê marnuje - pomniejsz
	...
	{
		m_Size = Size;
		// jeœli brakuje miejsca - powiêksz
...
{
	uint1 *ByteData = (uint1*)Data; // Bêdzie przesuwany
...
uint4 Hash_Calc::Calc(const void *Buf, uint4 BufLen)
{
	...
	uint1 *ByteData = (uint1*)Buf; // Bêdzie przesuwany
...
size_t BinDecoder::Read(void *Out, size_t Size)
{
	...
	if (m_Tolerance == DECODE_TOLERANCE_NONE)
	{
		while (Size > 0)
		{
			// Pierwszy bit - mo¿e go nie byæ jeœli to koniec strumienia
	...
	else if (m_Tolerance == DECODE_TOLERANCE_WHITESPACE)
	{
		while (Size > 0)
		{
			...
			for (;;)
			{
				...
				{
					// Nic - nowy obieg tej pêtli wewnêtrznej
				}
				else
					throw Error(ERRMSG_DECODE_INVALID_CHAR, __FILE__, __LINE__);
			}
...
bool BinDecoder::DecodeLength(uint *OutLength, const string &s, DECODE_TOLERANCE Tolerance)
{
	...
	// Pewne dodatkowe znaki s¹ tolerowane - trzeba wszystkie przejrzeæ i zliczyæ same cyfry
	else
	{
		uint DigitCount = 0;
...
bool BinDecoder::DecodeLength(uint *OutLength, const char *s, uint s_Length, DECODE_TOLERANCE Tolerance)
{
	...
	{
		// s bêdzie przesuwany. s_Length bêdzie zmniejszany.
...
{
	// DataLength bêdzie zmniejszane.
	// Out i InBytes bêd¹ przesuwane.
...
size_t HexDecoder::Read(void *Out, size_t Size)
{
	...
	if (m_Tolerance == DECODE_TOLERANCE_NONE)
	{
		while (Size > 0)
		{
			// Pierwsza cyfra - mo¿e jej nie byæ jeœli to koniec strumienia
			if (!m_CharReader.ReadChar(&Ch))
				break;
...
{
	// Ile pozosta³ych bajtów zalega jeszcze w buforze
	switch (m_BufIndex)
	{
	case 1:
...
{
	// Bêdê zmniejsza³ Size i przesuwa³ ByteData
	...
	// Pêtla przetwarza kolejne bajty
	while (Size > 0)
	{
		if (m_BufIndex == 2)
		{
			// Bufor pe³ny - mamy trzy bajty - dwa w m_Buf, trzeci w *ByteData. Czas siê ich pozbyæ.
			m_CharWriter.WriteChar( BASE64_CHARS[ m_Buf[0] >> 2 ] );
			m_CharWriter.WriteChar( BASE64_CHARS[ ((m_Buf[0] & 0x3) << 4) | (m_Buf[1] >> 4) ] );
...
bool Base64Decoder::ReadNextBuf()
{
	...
	if (m_Tolerance == DECODE_TOLERANCE_NONE)
	{
		...
		// B³êdne znaki lub '=' tam gdzie nie trzeba.
		if (Numbers[0] >= 0xFE) throw Error(ERRMSG_DECODE_INVALID_CHAR);
		if (Numbers[1] >= 0xFE) throw Error(ERRMSG_DECODE_INVALID_CHAR);
		if (Numbers[2] >= 0xFE) throw Error(ERRMSG_DECODE_INVALID_CHAR);
	...
	else
	{
		...
		for (;;)
		{
			...
			if (!m_CharReader.ReadChar(&Ch))
			{
				...
				// Jesteœmy na granicy czwórki znaków - OK, koniec
				if (NumberIndex == 0)
					return false;
				// Nie jesteœmy na granicy czwórki znaków - b³¹d, niedokoñczone dane
				else
...
size_t Base64Decoder::Read(void *Out, size_t Size)
{
	...
	uint1 *OutBytes = (uint1*)Out;
	// Size bêdzie zmniejszany. OutBytes bêdzie przesuwany.
...
bool Base64Decoder::DecodeLength(uint *OutLength, const string &s, DECODE_TOLERANCE Tolerance)
{
	...
	// Mog¹ byæ te¿ inne znaki - trzeba wszystkie przejrzeæ i policzyæ
	else
	{
		uint EqualCount = 0;
		uint DigitCount = 0;
...
{
	// Nie zmieœci siê w buforze
	if (m_Size + Size > m_Capacity)
		throw Error(Format("B³¹d zapisu do RingBuffer: Nie mo¿na zapisaæ # B - dane siê nie zmieszcz¹.") % Size, __FILE__, __LINE__);

	// Zmieœci siê ca³ej na koñcu
	if (Size <= m_Capacity - m_EndIndex)
	{
		CopyMem(&m_Buf[m_EndIndex], Data, Size);
		m_EndIndex += Size;
	}
	// Nie zmieœci siê na koñcu
	else {
		uint PartSize = m_Capacity - m_EndIndex;

		// Przepisz na koniec ile siê zmieœci
		CopyMem(&m_Buf[m_EndIndex], Data, PartSize);
		// Przepisz resztê na pocz¹tek
...
void RingBuffer::MustRead(void *Out, size_t Length)
{
	...
	if (m_Size < Length)
		throw Error(Format("Nie mo¿na odczytaæ # B z RingBuffer - nie ma tylu w buforze, jest tylko # B.") % Length % m_Size, __FILE__, __LINE__);
...
size_t RingBuffer::Skip(size_t MaxLength)
{
	...
	// Ustal ile bajtów pomin¹æ
	uint Length = std::min(MaxLength, m_Size);
	// Wszystko jest w jednym kawa³ku
	if (m_Capacity - m_BegIndex >= Length)
		m_BegIndex += Length;
	// Jest w dwóch kawa³kach
	else
	{
		uint PartSize = m_Capacity - m_BegIndex;
		m_BegIndex = Length - PartSize;
