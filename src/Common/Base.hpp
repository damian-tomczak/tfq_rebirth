...
// Niezale¿nie od systemu, sygnatura bie¿¹cej funkcji bêdzie w makrze __FUNCSIG__
#ifndef WIN32
...
// ¯e klasa nie ma konstruktora kopiuj¹cego ani operatora przypisania.
...
// Polityka zwalaniania, która robi: p->Release();
class ReleasePolicy
{
public:
	template <typename T>
...
// Jak wy¿ej, ale zamiast 0..1 zwraca liczbê z przedzia³u min..max.
...
ci¹g³a funkcja pseudolosowa i amplitudzie -1..1 i czêstotliwoœci 1.
...
// Half of the time is spent at the min, half at the max with instant transition
// between.
...
// Gradual steady increase from min to max over the period with an instant
...
// Gradual steady decrease from max to min over the period, with an instant
// return to max at the end.
...
public:
	...
	RandomGenerator(uint4 seed) : m_Seed(seed) { }
	...
	// Uwaga! Wygl¹da na to ¿e jest problem z RandUint(2) - wychodzi zawsze 0, nie wiem dlaczego.
	...
	// Ta konstrukcja to mój swobodny wymys³.
	uint1 RandByte()
	{
		uint4 u = RandUint();
		return (u >> 13) ^ u;
	...
	// generuje liczbê losow¹ rzeczywist¹ z przedzia³u 0..1
...
// Domyœlny generator liczb losowych do u¿ywania w w¹tku g³ównym i kiedy nie
// musi byæ deterministyczny
...
public:
	...
	string GetString() { string R; GetString(&R); return R; }
	// Zwraca unikatowy ³añcuch w formacie "<prefix>########", gdzie ######## to liczba szesnastkowa.
	void GetString(string *Out, const string &Prefix);
	string GetString(const string &Prefix) { string R; GetString(&R, Prefix); return R; }
};
