...
// Klasa Thread
	...
	Thread::~Thread()
	{
		assert(pimpl->Running == false && "Thread: Wywo³any destruktor dla w¹tku, który mo¿e jeszcze dzia³aæ (nie by³o wywo³ane Join).");
	}
	...
	void Thread::Start()
	{
		...
		if (pimpl->CompletionEvent == NULL)
			throw Win32Error("Nie mo¿na utworzyæ CompletionEvent.", __FILE__, __LINE__);
		...
		//	throw ErrnoError("Nie mo¿na utworzyæ w¹tku.", __FILE__, __LINE__);
	}
	...
	void Thread::Join()
	{
		...
		if (R == WAIT_FAILED)
			throw Win32Error("Nie mo¿na zaczekaæ na w¹tek.", __FILE__, __LINE__);
	...
	void Thread::Start()
	{
		...
		if (R != 0)
			throw ErrnoError(R, "Nie mo¿na utworzyæ w¹tku.", __FILE__, __LINE__);
	...
	void Thread::Join()
	{
		...
		if (R != 0)
			throw ErrnoError(R, "Nie mo¿na zaczekaæ na w¹tek.", __FILE__, __LINE__);
	}
...
// Klasa Mutex
		...
		pimpl(new Mutex_pimpl)
	{
		...
		if ((Flag & FLAG_WAIT_TIMEOUT) != 0)
		{
			...
			if (pimpl->Mutex == NULL)
				throw Win32Error("Nie mo¿na utworzyæ muteksa.", __FILE__, __LINE__);
		}
		// Sekcja krytyczna
		else
		{
		...
		pimpl(new Mutex_pimpl)
	{
		...
		if (R != 0)
			throw ErrnoError(R, "Nie mo¿na utworzyæ muteksa.", __FILE__, __LINE__);
	}
...
// Klasa Semaphore
		...
		pimpl(new Semaphore_pimpl)
	{
		...
		if (pimpl->SemaphoreHandle == NULL)
			throw Win32Error("Nie mo¿na utworzyæ semafora.", __FILE__, __LINE__);
	}
		...
		pimpl(new Semaphore_pimpl)
	{
		if (sem_init(&pimpl->s, 0, InitialValue) != 0)
			throw ErrnoError("Nie mo¿na utworzyæ semafora.", __FILE__, __LINE__);
	}
...
*/
		...
		pimpl(new Cond_pimpl)
	{
		...
		if (R != 0)
			throw ErrnoError(R, "Nie mo¿na utworzyæ zmiennej kondycyjnej.", __FILE__, __LINE__);
	}
...
// Klasa Barrier
		...
		pimpl(new Barrier_pimpl)
	{
		...
		if (R != 0)
			throw ErrnoError(R, "Nie mo¿na utworzyæ bariery.", __FILE__, __LINE__);
	}
...
// Klasa Event
		...
		pimpl(new Event_pimpl)
	{
		pimpl->E.reset(CreateEvent(
			...
			NULL));
		if (pimpl->E == NULL)
			throw Error("Nie mo¿na utworzyæ zdarzenia.", __FILE__, __LINE__);
	}
