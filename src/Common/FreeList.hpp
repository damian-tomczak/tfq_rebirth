...
public:
	...
	{
		assert(Capacity > 0);
		assert(sizeof(T) >= sizeof(FreeBlock) && "FreeList nie dzia³a dla tak ma³ych elementów.");
	...
	~FreeList()
	{
		assert(m_FreeCount == m_Capacity && "FreeList usuniêty zanim zwolnione wszystkie jego elementy.");
		delete [] m_Data;
	}
...
public:
	// BlockCapacity to d³ugoœæ jednego bloku, w elementach
	...
	void Delete(T *x)
	{
		...
		assert(0 && "DynamicFreeList.Delete: Komórka nie nale¿y do ¿adnego z bloków tej listy.");
	}
