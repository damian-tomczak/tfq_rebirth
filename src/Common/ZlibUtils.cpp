 ...
 * ZlibUtils - Modu³ wspmagaj¹cy biblioteki zlib [modu³ przenoœny, wymaga biblioteki zlib]
	...
	m_Stream(a_Stream)
{
	...
	if (R != Z_OK)
		throw ZlibError(R, "Nie mo¿na zainicjalizowaæ kompresji zlib", __FILE__, __LINE__);
}
	...
	m_Stream(a_Stream)
{
	...
	if (R != Z_OK)
		throw ZlibError(R, "Nie mo¿na zainicjalizowaæ kompresji gzip", __FILE__, __LINE__);
	...
	if (R != Z_OK)
		throw ZlibError(R, "Nie mo¿na ustawiæ nag³ówka gzip", __FILE__, __LINE__);
}
...
size_t ZlibCompressionStream::Compress(void *OutData, size_t BufLength, const void *Data, size_t DataLength, int Level)
{
	...
	if (R != Z_OK)
		throw ZlibError(R, "Nie mo¿na skompresowaæ danych zlib", __FILE__, __LINE__);
	return DestLen;
}
	...
	m_Stream(a_Stream)
{
	...
	if (Gzip)
	{
		...
		if (R != Z_OK)
			throw ZlibError(R, "Nie mo¿na zainicjalizowaæ dekompresji gzip", __FILE__, __LINE__);
	...
	else
	{
		...
		if (R != Z_OK)
			throw ZlibError(R, "Nie mo¿na zainicjalizowaæ dekompresji zlib", __FILE__, __LINE__);
	}
...
//		throw ZlibError(R, "Nie mo¿na rozpocz¹æ dekompresji zlib", __FILE__, __LINE__);
}
...
size_t ZlibDecompressionStream_pimpl::Read(void *Out, size_t MaxLength)
{
	...
	for (;;)
	{
		...
		if (m_ZStream.avail_in == 0)
		{
			...
			if (m_InBufLen == 0)
				throw Error("B³¹d strumienia dekompresji zlib: Niedokoñczone dane.", __FILE__, __LINE__);
			m_ZStream.next_in = (Bytef*)&m_InBuf[0];
			m_ZStream.avail_in = m_InBufLen;
		}
...
size_t ZlibDecompressionStream::Decompress(void *OutData, size_t BufLength, const void *Data, size_t DataLength)
{
	...
	if (R != Z_OK)
		throw ZlibError(R, "Nie mo¿na rozkompresowaæ danych zlib", __FILE__, __LINE__);
	return DestLen;
}
	...
	pimpl(new GzipFileStream_pimpl)
{
	...
	{
		if (errno == 0)
			throw ZlibError(Z_MEM_ERROR, "Nie mo¿na otworzyæ pliku gzip: " + FileName, __FILE__, __LINE__);
		else
			throw ErrnoError("Nie mo¿na otworzyæ pliku gzip: " + FileName, __FILE__, __LINE__);
	}
	...
	if (Mode == GZFM_READ)
	{
		...
		if (R == -1)
			throw Error("Nie mo¿na odczytaæ pierwszego bajtu z pliku gzip", __FILE__, __LINE__);
		pimpl->m_End = (R == 0);
	}
}
...
void GzipFileStream::Write(const void *Data, size_t Size)
{
	...
	if (Written != Size)
		throw Error(Format("Nie mo¿na zapisaæ do pliku gzip - zapisano #/# B") % Written % Size, __FILE__, __LINE__);
}
...
size_t GzipFileStream::Read(void *Out, size_t MaxLength)
{
	...
	*Out2 = pimpl->OneCharBuf;
	...
	if (BytesRead == -1)
		throw Error(Format("Nie mo¿na odczytaæ # bajtów z pliku gzip") % (MaxLength+1), __FILE__, __LINE__);
	...
	else
	{
		int R = gzread(pimpl->File, &pimpl->OneCharBuf, 1);
		if (R == -1)
			throw Error("Nie mo¿na odczytaæ bajtu z pliku gzip", __FILE__, __LINE__);
		pimpl->m_End = (R == 0);
	}
